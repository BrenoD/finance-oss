...
//  WEEKLY TAB
// ─────────────────────────────────────────────────────────────────────────────
function WeeklyTab({subs,expenses,income,user}) {
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      if (DB_READY) {
        const rows = await sb.select("weekly_entries",`user_id=eq.${user?.id}&week_key=eq.${selWeek}&order=created_at.asc`);
        setEntries(rows);
      } else {
...
function FinanceApp({ user, onSignOut, joinedCouple }) {
      ...
      try {
        if (DB_READY) {
          const [subsRows, expRows, settingRows] = await Promise.all([
            sb.select("subscriptions",`user_id=eq.${user.id}&order=created_at.asc`),
            sb.select("expenses",`user_id=eq.${user.id}&order=created_at.asc`),
            sb.select("settings",`user_id=eq.${user.id}&key=eq.income`),
          ]);
          // fetch payment history for each sub
          const payments = await sb.select("sub_payments",`user_id=eq.${user.id}&order=created_at.desc`);
          ...
          const [peName,peVal,peEnabled] = await Promise.all([
            sb.select("settings",`user_id=eq.${user.id}&key=eq.partnerName`),
            sb.select("settings",`user_id=eq.${user.id}&key=eq.partnerIncome`),
            sb.select("settings",`user_id=eq.${user.id}&key=eq.partnerEnabled`),
          ]);
  ...
  const addSub = async () => {
    ...
    try {
      const payload = {user_id:user.id, title:newSub.title, amount:parseFloat(newSub.amount),
        ci:subs.length%ACCENTS.length, logo_err:false};
      if (DB_READY) {
        const [row] = await sb.insert("subscriptions", payload);
        let history = [];
        if (subPayDate) {
          const [prow] = await sb.insert("sub_payments",{user_id:user.id,sub_id:row.id,paid_date:subPayDate.date,paid_time:subPayDate.time});
  ...
  const markPaid = async (id) => {
    ...
    try {
      if (DB_READY) {
        await sb.insert("sub_payments",{user_id:user.id,sub_id:id,paid_date:d,paid_time:t});
      }
  ...
  const addExp = async () => {
    ...
    const payload = {
      user_id:user.id, label:newExp.label, amount:parseFloat(newExp.amount),
                  ...
                  editPartner?(
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      <input type="number" value={partnerIncome||""} autoFocus
                        ...
                        onBlur={()=>{setEditPartner(false);savePartner(partnerName,parseFloat(partnerIncome)||0,partnerEnabled);}}
                        onKeyDown={e=>e.key==="Enter"&&(setEditPartner(false),savePartner(partnerName,parseFloat(partnerIncome)||0,partnerEnabled))}
